 ...
 */
export const checkAndUpdateUserLevel = async (userId: string): Promise<{ level: number; leveledUp: boolean; newLevel?: number }> => {
  try {
    ...
    if (levels.length === 0) {
      ...
      const currentLevel = fallbackLevels.find(level => 
        user.experience >= level.requiredExperience && 
 ...
 */
export const getNextLevelRequirements = async (userId: string): Promise<{ currentLevel: number; nextLevel?: number; requiredXP?: number; progress?: number }> => {
  try {
    const user = await User.findById(userId);
    if (!user) {
      throw new Error('User not found');
