...
async function seedAdvancedData() {
  try {
    ...
    const createdAdvancedVocabularies = await Vocabulary.insertMany(advancedVocabularies);
    ...
    const createdAdvancedTests = await Test.insertMany(advancedTests);
    ...
    const createdAdditionalReports = await Report.insertMany(reportData);
