...
async function seedCompleteData() {
  try {
    ...
    const createdTopics = await Topic.insertMany(completeTopics);
    console.log(`Created ${createdTopics.length} topics`);

    // Seed Levels
    console.log('Seeding levels...');
    const createdLevels = await Level.insertMany(completeLevels);
    console.log(`Created ${createdLevels.length} levels`);

    // Seed Users
    console.log('Seeding users...');
    const createdUsers = await User.insertMany(completeUsers);
    console.log(`Created ${createdUsers.length} users`);

    // Seed Vocabularies
    console.log('Seeding vocabularies...');
    const createdVocabularies = await Vocabulary.insertMany(completeVocabularies);
    console.log(`Created ${createdVocabularies.length} vocabularies`);

    // Seed Tests
    console.log('Seeding tests...');
    const createdTests = await Test.insertMany(completeTests);
    console.log(`Created ${createdTests.length} tests`);

    // Seed Proficiency Tests
    console.log('Seeding proficiency tests...');
    const createdProficiencyTests = await ProficiencyTest.insertMany(completeProficiencyTests);
    console.log(`Created ${createdProficiencyTests.length} proficiency tests`);

    // Seed Competitions
    console.log('Seeding competitions...');
    const createdCompetitions = await Competition.insertMany(completeCompetitions);
    ...
    const createdCompetitionResults = await CompetitionResult.insertMany(competitionResultsData);
    ...
    const createdUserVocabularies = await UserVocabulary.insertMany(userVocabularyData);
