...
const daysFromNow = (d: number, hour = 20, min = 0) => {
  const date = new Date();
...
async function upsertUser(name: string, email: string, role: 'teacher' | 'user'): Promise<any> {
  let user: any = await User.findOne({ email });
  if (!user) {
    user = new (User as any)({ name, email, password: PASSWORD, role });
    await user.save();
...
const mcq = (question: string, options: string[], correctAnswer: number) => ({
...
async function run() {
  ...
  if (!admin) { console.error('Chưa có admin. Chạy seed:admin trước.'); process.exit(1); }
  const adminId = admin._id;

  // 1. Clean previous demo data (keep demo users, refresh everything else)
  const oldClasses = await LearningClass.find({ name: { $regex: '^\\[Demo\\]' } }).select('_id');
  const oldClassIds = oldClasses.map(c => c._id);
  await Promise.all([
    ClassSession.deleteMany({ classId: { $in: oldClassIds } }),
  ...
  // 2. Users
  const teachers: any[] = [];
  for (const t of TEACHERS) teachers.push(await upsertUser(t.name, t.email, 'teacher'));
  const students: any[] = [];
  ...
  const buildExercises = (test: any, count: number) =>
    (test.questions || []).slice(0, count).map((q: any) => ({
  ...
  const vocabPool: any[] = await Vocabulary.find({ 'questions.0': { $exists: true } }).select('_id word').limit(60);
  if (vocabPool.length < 10) console.warn('Cảnh báo: ít từ vựng có câu khảo.');
  const pickVocab = (offset: number, n: number) => vocabPool.slice(offset, offset + n).map((v: any) => v._id);
  ...
  for (let ci = 0; ci < classDefs.length; ci++) {
      ...
      for (let sIdx = 0; sIdx < classStudents.length; sIdx++) {
        ...
        if (sIdx % 10 < 7) {
          ...
          await ClassFeedback.create({
            ...
            lessonDifficulty: ['just_right', 'a_bit_hard', 'too_easy', 'very_hard'][sIdx % 4] as any,
            vocabularyMemory: ['good', 'partial', 'weak', 'need_review'][sIdx % 4] as any,
            grammarUnderstanding: ['clear', 'partial', 'unclear', 'need_reteach'][sIdx % 4] as any,
    ...
    if (futureSession) {
      for (let k = 0; k < Math.min(2, classStudents.length); k++) {
        await ClassLeaveRequest.create({
...
run().catch(async (err) => { console.error('Seed error:', err); try { await mongoose.connection.close(); } catch {} process.exit(1); });
